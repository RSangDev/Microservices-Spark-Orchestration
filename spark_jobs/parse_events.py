...
def create_spark_session(run_id: str) -> SparkSession:
    return (
        ...
        .appName(f"parse_events_{run_id}")
...
def validate_dataframe(df, service_name: str, spark: SparkSession) -> tuple:
    ...
    validation_rules = {
        ...
        "not_empty_event_type": (F.col("event_type").isNotNull()) & (F.col("event_type") != ""),
    }

    # Adiciona regras específicas por serviço
    if service_name == "payments-service":
        validation_rules["positive_amount"] = F.col("amount") > 0
        validation_rules["valid_currency"] = F.col("currency").isin("BRL", "USD", "EUR", "GBP")
    ...
    log.info(
        ...
        service_name, total, valid_count, invalid_count, metrics["validity_rate"] * 100,
    )
...
def main():
    args = parse_args()
    services_to_process = (
        list(SCHEMAS.keys()) if args.services == "all"
        else args.services.split(",")
    )

    spark = create_spark_session(args.run_id)
    spark.sparkContext.setLogLevel("WARN")

    log.info("🚀 Iniciando parse_events | run_id: %s | modo: %s", args.run_id, args.mode)
    ...
    for service_name in services_to_process:
        ...
        try:
            ...
            if duplicates_removed > 0:
                log.warning("%s: %d duplicatas removidas", service_name, duplicates_removed)

            # Validação
            df_valid, df_invalid, metrics = validate_dataframe(df_deduped, service_name, spark)
            ...
            if df_invalid.count() > 0:
                ...
                log.warning("%s: %d registros em quarentena", service_name, df_invalid.count())
    ...
    combined = reduce(
        lambda a, b: a.select(*[c for c in common_cols if c in a.columns])
                       .unionByName(
    ...
    log.info(
        ...
        total_valid / (total_valid + total_invalid) * 100 if (total_valid + total_invalid) > 0 else 0,
    )
