...
@pytest.mark.skipif(sys.platform == "win32", reason="fcntl nao existe no Windows — rode no container")
class TestDagStructure:
    ...
    def test_orchestration_required_tasks(self, dagbag):
        ...
        required = {"start", "route_based_on_health", "end", "register_execution_metrics"}
...
class TestDagSyntax:
    ...
    def _get_dag_files(self):
        ...
        return [os.path.join(dags, f) for f in os.listdir(dags) if f.endswith(".py") and not f.startswith("_")]
    ...
    def test_dag_ids_in_source(self):
        ...
        for path in self._get_dag_files():
            ...
            if name in expected:
                with open(path, encoding="utf-8") as f:
                    source = f.read()
                assert expected[name] in source, f"dag_id '{expected[name]}' não encontrado em {name}"
...
class TestMicroserviceHealthSensor:
    ...
    def test_poke_wrong_status(self, mock_get):
        mock_get.return_value = MagicMock(status_code=200, json=lambda: {"status": "starting"})
    ...
    def test_poke_dependencies_all_healthy(self, mock_get):
        mock_get.return_value = MagicMock(
            status_code=200,
            json=lambda: {"status": "healthy", "dependencies": {"database": {"status": "healthy"}, "cache": {"status": "healthy"}}},
        )
        assert self._make_sensor(required_dependencies=["database", "cache"]).poke({}) is True

    @patch("requests.get")
    def test_poke_dependency_unhealthy(self, mock_get):
        mock_get.return_value = MagicMock(
            status_code=200,
            json=lambda: {"status": "healthy", "dependencies": {"database": {"status": "healthy"}, "cache": {"status": "degraded"}}},
        )
        assert self._make_sensor(required_dependencies=["database", "cache"]).poke({}) is False
...
class TestServiceRegistryHook:
    ...
    def test_list_active_services(self, mock_conn, mock_get):
        ...
        services_response = MagicMock(
            status_code=200,
            json=lambda: {"orders-service": ["event-producer"], "payments-service": ["event-producer"]},
        )
        detail_response = MagicMock(
            status_code=200,
            json=lambda: [{"ServiceAddress": "10.0.0.1", "ServicePort": 8001,
                           "ServiceMeta": {"events_endpoint": "/api/v1/events/export", "priority": "high"}}],
        )
        ...
        services = ServiceRegistryHook(conn_id="service_registry").list_active_services(tag="event-producer")
...
class TestSparkMicroserviceOperator:
    ...
    def test_build_command_basics(self):
        cmd = self._make_op(num_executors=2, executor_memory="2g")._build_spark_submit_cmd()
    ...
    def test_build_command_with_conf(self):
        cmd    = self._make_op(conf={"spark.executor.cores": "2"})._build_spark_submit_cmd()
    ...
    def test_build_command_with_args(self):
        cmd = self._make_op(application_args=["--input", "s3://bucket/data"])._build_spark_submit_cmd()
        assert "--input" in cmd and "s3://bucket/data" in cmd

    def test_extract_app_id(self):
        line = "INFO SparkContext: Submitted application: application_1704067200000_0042"
...
class TestSparkJobs:
    ...
    def test_parse_events_args(self):
        sys.argv = ["parse_events.py", "--input", "s3://bucket/raw", "--output", "s3://bucket/parsed",
                    "--run-id", "test_001", "--mode", "normal", "--services", "orders-service"]
    ...
    def test_parse_events_schemas_complete(self):
        ...
        expected = {"orders-service", "payments-service", "inventory-service", "shipping-service"}
    ...
    def test_aggregate_metrics_args(self):
        sys.argv = ["aggregate_metrics.py", "--input", "s3://bucket/enriched",
                    "--output", "s3://bucket/metrics", "--run-id", "test_001", "--window", "1h"]
